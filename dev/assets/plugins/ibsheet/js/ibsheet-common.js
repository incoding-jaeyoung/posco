...
//    Alternate: 2, // 홀짝 행에 대한 배경색 설정
    ...
    MaxSort: 1, // 최대 소팅 가능 컬럼수(4개 이상인 경우 느려질 수 있음)
  ...
  //border radius 를 위해 상하단에 행을 얇게 만들자
  Solid:[
...
_IBSheet.onBeforeCreate = function(init){
  // 헤더행이 한줄인 경우, 행의 높이를 30으로.. 두줄 이상인 경우 25px
  if( !Array.isArray(init.options.Cols[0].Header) || init.options.Cols[0].Header.length === 1){
    init.options.Def = init.options.Def||{};
    init.options.Def.Header = init.options.Def.Header || {};
    init.options.Def.Header["Height"] = 30;
  }
  var headerCheck = function(col){
    if(col.Header === "" || col.Header?.Value === ""){
      col.Header = {};
      col.Header["Value"] = "";
      col.Header["IconAlign"] = "Center";
    }
  }
  // 헤더 체크박스를 가운데로 표시
  init.options.Cols.forEach(col => {    headerCheck(col);  })
  init.options?.LeftCols?.forEach(col => {    headerCheck(col);  })
  init.options?.RightCols?.forEach(col => {    headerCheck(col);  })
